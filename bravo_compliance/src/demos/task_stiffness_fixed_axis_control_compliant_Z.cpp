...
void program_loop(std::shared_ptr<airbus_joystick_bravo_twist_ee> airbus_joy, std::shared_ptr<bravo7_handler<double>> bravo){     
    ...
    double MAX_CURRENT_mA = 2500.0; // Max continuous current for the Bravo7 motors
    double MAX_MANIPULABILITY = 25.0; // Threshold for low manipulability
    ...
    Eigen::Vector3d Kxyz(500.0, 5000.0, 5000.0); // Stiffness gains for 3D control
    Eigen::MatrixXd positionStiffness = Kxyz.asDiagonal();
    Eigen::Vector3d Dxyz(10.0, 200.0, 200.0); // Damping gains for 3D control
