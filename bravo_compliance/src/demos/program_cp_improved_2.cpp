...
void program_loop(std::shared_ptr<airbus_joystick_bravo_twist_ee> airbus_joy, std::shared_ptr<bravo7_handler<double>> bravo){     
    ...
    Eigen::VectorXd mA_stiction_stribeck_custom(6);
    mA_stiction_stribeck_custom.setZero();
    ...
    while (rclcpp::ok()) {
        ...
        if ((bravo->compute_manipulability()>MAX_MANIPULABILITY) || (ratio_force_manipulability > MAX_RATIO_FORCE_ELLIPSOID) || goHome){
            ...
            if (airbus_joy->enableBaseMotion){
                Kxyz << 2000.0, 5000.0, 5000.0;
                positionStiffness = Kxyz.asDiagonal();
                Dxyz << 200.0, 300.0, 300.0; // Damping gains for 3D control
                ...
                std::cout << "Velocity commands (x,y,z): " << vel_x << ", " << vel_y << ", " << vel_z << std::endl;
                std::cout << "Position ref (x,y,z): " << ref_ee_pos[0] << ", " << ref_ee_pos[1] << ", " << ref_ee_pos[2] << std::endl;
                ...
                Kxyz << 50.0, 5000.0, 5000.0;
                positionStiffness = Kxyz.asDiagonal();
                Dxyz << 5.0, 200.0, 200.0; // Damping gains for 3D control
                positionDamping = Dxyz.asDiagonal(); 
                //* FORCE ESTIMATION and UPDATE VELOCITY REFERENCE (ADMITTANCE CONTROL IN Z)
                double factor = 1.0; 
            ...
            Eigen::VectorXd joint_velocity_friction(6), bravo_vel_feedback(6);
            bravo_vel_feedback = bravo->get_bravo_joint_velocities();
            if ((bravo_vel_feedback.array().abs() < 0.04).all()){
            ...
            if (airbus_joy->enableBaseMotion)
            {
                mA_stiction_stribeck_custom   =  compute_I_ff_stribeck_smooth(joint_velocity_friction, coulomb_custom, stiction_custom, 0.045, 100.0, 0.03);
            }
            else{
                mA_stiction_stribeck_custom   =  compute_I_ff_stribeck(bravo->get_bravo_joint_velocities(), coulomb_custom, stiction_custom, qd_stiction);
            }
            //Eigen::VectorXd mA_stiction_stribeck_custom   =  compute_I_ff_stribeck_smooth(joint_velocity_friction, coulomb_custom, stiction_custom, 0.045, 100.0, 0.03);
            //Eigen::VectorXd mA_stiction_stribeck_custom   =  compute_I_ff_stribeck(bravo->get_bravo_joint_velocities(), coulomb_custom, stiction_custom, qd_stiction);
