...
void program_loop(std::shared_ptr<airbus_joystick_bravo5_CP> airbus_joy, std::shared_ptr<bravo_handler<double>> bravo){     
    //& PARAMETERS
    ...
    double MAX_RATIO_FORCE_ELLIPSOID = 0.2;
    ...
    stiffness_controller.set_pos_stiffness(Eigen::Vector3d(50.0, 1000.0, 200.0));
    stiffness_controller.set_pos_damping(Eigen::Vector3d(1.0, 100.0, 20.0));
    stiffness_controller.set_gain_force(Eigen::Vector3d(0.03, 0.0, 0.0));
    stiffness_controller.set_nominal_vel(Eigen::Vector3d(0.05, 0.0, 0.0));
    stiffness_controller.set_desired_force(Eigen::Vector3d(1.5, 0.0, 0.0)); // Desired force in Z
    stiffness_controller.set_ref_ee_position(bravo->kinodynamics.FK_ee_pos(ref_joint_pos));

    double LOOP_FREQUENCY = 200.0;  // Hz (change as needed)
