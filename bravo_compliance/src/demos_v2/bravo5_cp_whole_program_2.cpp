...
void program_loop(std::shared_ptr<airbus_joystick_bravo5_CP> airbus_joy, std::shared_ptr<bravo_handler<double>> bravo){     
    ...
    stiffness_controller.set_desired_force(Eigen::Vector3d(600.0, 0.0, 0.0)); // Desired force in Z
    ...
    while (rclcpp::ok()) {
        ...
        mA_pd_current_cmd[3] = general_utils::VAL_SAT<double>(mA_pd_current_cmd[3], MAX_CURRENT_mA_GO_HOME, -MAX_CURRENT_mA_GO_HOME);
