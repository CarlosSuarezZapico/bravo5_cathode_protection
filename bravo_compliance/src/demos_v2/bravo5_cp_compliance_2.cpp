...
void program_loop(std::shared_ptr<airbus_joystick_bravo_twist_ee> airbus_joy, std::shared_ptr<bravo_handler<double>> bravo){     
    ...
    Eigen::VectorXd mA_joint_current_cmd(4), joint_velocity_friction(4), mA_stiction_stribeck_custom(4), coulomb_custom(4), stiction_custom(4), qd_stiction(4);
    ...
    if (bravo->compute_manipulability_position() < MAX_MANIPULABILITY){
        std::cout << "Initial manipulability condition number OK, setting ref" << std::endl;
        ref_joint_pos = bravo->get_bravo_joint_states();
        stiffness_controller.set_pos_stiffness(Eigen::Vector<double, 3>(70.0, 1000.0, 1000.0));
        ...
        std::cerr << "Manipulability condition number: " << bravo->compute_manipulability_position() << std::endl;
        std::cerr << "Joint Position: " << bravo->get_bravo_joint_states().transpose() << std::endl;
        rclcpp::shutdown();
        return;
    }
