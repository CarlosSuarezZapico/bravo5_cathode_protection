...
void program_loop(std::shared_ptr<airbus_joystick_bravo_twist_ee> airbus_joy, std::shared_ptr<bravo7_handler<double>> bravo){     
    ...
    Eigen::Vector3d Kxyz(1000.0, 5000.0, 5000.0); // Stiffness gains for 3D control
    Eigen::MatrixXd positionStiffness = Kxyz.asDiagonal();
    Eigen::Vector3d Dxyz(20.0, 80.0, 80.0); // Damping gains for 3D control
    ...
    HOME  << 3.14, 2.478, 1.797, 0.0, 2.005, 0.0; 
    ...
    while (rclcpp::ok()) {
        ...
        if ((bravo->compute_manipulability()>25) || goHome){
            ...
            Eigen::VectorXd error = bravo->go_to_configuration(HOME, 0.3 * Eigen::VectorXd::Ones(6), elapsed_seconds.count());            
