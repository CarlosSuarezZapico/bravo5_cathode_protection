...
void program_loop(std::shared_ptr<airbus_joystick_bravo_twist_ee> airbus_joy, std::shared_ptr<bravo7_handler<double>> bravo){
    ...
    force_controllerZ->set_admittance_parameters(2.0, 250);

    bravo->set_bravo_frequency_packet_exchange(500); //! Set frequency of requests
    ...
    while (rclcpp::ok()) {
        if (bravo->compute_manipulability()>25){
            ...
            force_controllerX->md_admittance_force_control_loop(0.005, 3.0, -FT[0]);
            force_controllerY->md_admittance_force_control_loop(0.005, 3.0, -FT[1]);
            ...
            bravo->moveCmdTCPLocalTwist(reduced_vel_factor*twist_joy, 0.0005); //! I DONT LIKE THIS INSTRUCTION HERE
        }
