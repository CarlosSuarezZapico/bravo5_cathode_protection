...
void teleop_bravo_sim(std::shared_ptr<airbus_joystick_bravo_twist_ee> airbus_joy, std::shared_ptr<bravo7_handler<double>> bravo, std::shared_ptr<force_control<double>> force_controller){
    force_controller->set_admittance_parameters(1.5, 250); //set virtual mass and damping (2,300) (1.5, 250)
    ...
    while (rclcpp::ok()) {
        if (bravo->compute_manipulability()>25){
            ...
            force_controller->md_admittance_force_control_loop(0.005, 8, -forceZ);
            double vel = 0.0;
            if (abs(forceZ)<3.0){
                vel = 0.07;
            }
