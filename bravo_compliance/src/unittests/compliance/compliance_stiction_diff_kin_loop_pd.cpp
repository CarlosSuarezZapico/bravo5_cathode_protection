...
void program_loop(std::shared_ptr<airbus_joystick_bravo_twist_ee> airbus_joy, std::shared_ptr<bravo7_handler<double>> bravo){     
    ...
    pos_gains << 4500.0, 4500.0, 4500.0, 3000.0, 3000.0, 3000.0;
    ...
    while (rclcpp::ok()) {
        if (bravo->compute_manipulability()>25){
            ...
            std::tie(high_cond, joint_position_cmd, joint_velocity_cmd) = bravo->twist_diff_kin_2(twist_fixed, 0.02);
            ...
            torque_cmd = pos_gains.array() * (bravo->signedAngleDistance(joint_position_cmd, bravo->get_bravo_joint_states())).array() + 
                        vel_gains.array() * (bravo->signedAngleDistance(joint_velocity_cmd, bravo->get_bravo_joint_velocities())).array() +
                        stiction.array() * smoothSignEigen(pos_err, 0.0015);
