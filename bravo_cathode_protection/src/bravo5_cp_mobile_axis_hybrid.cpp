...
bool program_loop(std::shared_ptr<airbus_joystick_bravo5_CP> airbus_joy,
    ...
    Eigen::MatrixXd jacobian3x4_fixed       = Eigen::MatrixXd::Zero(3,4);
    ...
    while (rclcpp::ok()) {
        ...
        jacobian3x4_fixed         = bravo->kinodynamics.fixedJacobian(joint_postion_fdb).topRows(3);
        ...
        switch(current_state) {
            ...
            case StateMachine::TELEOP: {
                ...
                const Eigen::Vector<double, 4> sim_joint_vel_integration = jacobian3x4_fixed.colPivHouseholderQr().solve(twist_joy_fixed); //! MAYBE BETTER USE DAMPING 
                sim_finish_integration_time = std::chrono::steady_clock::now();
