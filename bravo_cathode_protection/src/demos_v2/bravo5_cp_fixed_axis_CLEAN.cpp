...
void program_loop(std::shared_ptr<airbus_joystick_bravo5_CP> airbus_joy, std::shared_ptr<bravo_handler<double>> bravo, const StiffnessJsonParams& stiff_params){     
    ...
    while (rclcpp::ok()) {
        ...
        switch (current_state) {
            case StateMachine::GO_HOME:
                    if (prev_state != StateMachine::HOME_WAITING){      
                        std::cout << "Switching to GO_HOME" << std::endl;   
                    }
            ...
            case StateMachine::TELEOP:
                    ...
                    Eigen::Vector<double, 4> sim_joint_vel_integration = jacobian3x4.colPivHouseholderQr().solve(twist_joy_fixed); //! MAYBE BETTER USE DAMPING 
        ...
        if ( (manipulability > MAX_MANIPULABILITY) || (!arrived2HOME) || (airbus_joy->goHome)){
            ...
            if (perform_reading){    // ! I DONT LIKE THIS
                MAKE_READING = true;
            }
        }
