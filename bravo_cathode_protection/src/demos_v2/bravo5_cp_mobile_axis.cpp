...
void program_loop(std::shared_ptr<airbus_joystick_bravo5_CP> airbus_joy,
    ...
    while (rclcpp::ok()) {
        ...
        if (force_go_home){
            ...
            jacobian3x4 = bravo->kinodynamics.localJacobian(sim_joint_whole_integration).topRows(3);
            ...
            const Eigen::Vector3d pos_error_mobile =
                current_ee_rot.transpose() * (stiffness_controller.get_ref_ee_position() - current_ee_pos);
