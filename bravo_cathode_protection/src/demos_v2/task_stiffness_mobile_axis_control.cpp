...
void program_loop(std::shared_ptr<airbus_joystick_bravo_twist_ee> airbus_joy, std::shared_ptr<bravo7_handler<double>> bravo){     
    ...
    Eigen::Vector3d Kxyz(3000.0, 3000.0, 900.0); // Stiffness gains for 3D control
    ...
    while (rclcpp::ok()) {
        ...
        Eigen::Vector3d position_error_ee = current_ee_rot.transpose() * (ref_ee_pos - current_ee_pos);
        //!ORIENTATION ERROR 
        // compute R_err mapping current -> reference (i.e. how to rotate current into reference)
        Eigen::Matrix3d R_err_ee = current_ee_rot.transpose() * ref_ee_rot;
        Eigen::Vector3d orientation_error;
        orientation_error << 
            0.5 * (R_err_ee(2,1) - R_err_ee(1,2)),
            0.5 * (R_err_ee(0,2) - R_err_ee(2,0)),
            0.5 * (R_err_ee(1,0) - R_err_ee(0,1));
        ...
        task_space_stiffness.head<3>() = positionStiffness * position_error_ee;
