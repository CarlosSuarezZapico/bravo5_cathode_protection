...
{ 
    ...
    enum control_mode_states {standby=0x00, disable=0x01, joint_position_mode=0x02, joint_velocity_mode=0x03, joint_current_mode=0x04, joint_torque_mode=0x0B, local_twist_ee_mode=5}; // Bravo list of control modes
    ...
    template <Floating32or64 T_data> 
        class bravo_udp {
            private:
                ...
                control_mode_states control_mode_state;    // Last mode reported by arm feedback
                control_mode_states command_mode_state;    // Mode used to encode outgoing commands
                ...
                std::atomic<bool> diagnosis{true};
                ...
                T_data request_time_delay_sec  = 0.0005;   // Default requesting time sampling
                ...
                //&SAFETY VARIABLES
                std::atomic<int> MAX_TIME_WITHOUT_FBD_ms{300}; 
                ...
                uint8_t *encoded_jointFbd_request = nullptr;
                uint8_t *encoded_single_jointCmd_request = nullptr;
                uint8_t *encoded_localTwisteeCmd_request = nullptr;

                //& NEW
                size_t size_feedback_request;
                uint8_t *encoded_feedback_request = nullptr;

                size_t size_whole_jointCmd_request; 
                uint8_t *encoded_whole_jointCmd_request = nullptr;

                size_t size_jointCmd_feedback_request;
                uint8_t *encoded_jointCmd_feedback_request = nullptr;

                size_t size_taskCmd_feedback_request;
                uint8_t *encoded_taskCmd_feedback_request = nullptr;
            ...
            //& METHODS        
                ...
                std::vector<T_data> get_bravo_joint_states() const;

                std::vector<T_data> get_bravo_joint_currents() const;

                std::vector<T_data> get_bravo_joint_velocities() const;

                std::vector<T_data> get_bravo_joint_torques() const;
                ...
                void set_max_time_without_fdb(int timeout_ms);
