...
template <FloatingPoint T>
     void stiffness_control_position<T>::compute_motion(Eigen::Vector<T, 3> exerted_force){   
          ...
          vel_cmd = nominal_velocity + gain_force.cwiseProduct(desired_force - exerted_force);   
          for (int i = 0; i < 3; ++i){
               vel_ee(i) = bravo_utils::VAL_SAT<T>(vel_cmd(i), MAX_TASK_VEL(i), -MAX_TASK_VEL(i));
          }
          T duration = bravo_utils::VAL_SAT<T>(std::chrono::duration<T>(std::chrono::high_resolution_clock::now() - last_computed_action).count(), MIN_SAMPLING_TIME, 0.0);
          debug_duration = duration; 
          //& MOBILE AXIS INTEGRATION
          if (reference_motion_in_local_frame){
               ref_ee_pos += local_to_world_rotation * vel_ee * duration;
          }
          //& FIXED AXIS INTEGRATION
          else{
