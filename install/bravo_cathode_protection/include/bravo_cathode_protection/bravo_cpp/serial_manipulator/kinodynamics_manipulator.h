...
public:
    ...
    Scalar compute_manipulability(const VectorX& q){
            MatrixX jacobian = localJacobian(q);
            Scalar cond_number = cond_arm(jacobian);
            return cond_number;
        }
    
    Scalar compute_manipulability_position(const VectorX& q){
            MatrixX Jacobian = fixedJacobian(q);
            MatrixX jacobian_pos = Jacobian.topRows(3);
            Scalar cond_number = cond_arm(jacobian_pos);
            return cond_number;
        }
